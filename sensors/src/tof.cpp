...
// mode.
bool VL53L0X::init(bool bLongRangeMode, bool io_2v8)
{
  ...
  if (bLongRangeMode)
  {
	writeReg16Bit(FINAL_RANGE_CONFIG_MIN_COUNT_RATE_RTN_LIMIT, 13); // 0.1
...
int VL53L0X::tofGetModel(int *model, int *revision)
{
	...
	if (model)
	{
		ucTemp[0] = IDENTIFICATION_MODEL_ID;
	...
	if (revision)
	{
		ucTemp[0] = IDENTIFICATION_REVISION_ID;
...
bool VL53L0X::writeReg32Bit(uint8_t ucAddr, uint32_t usValue)
{
	...
	ucTemp[0] = ucAddr;
    ucTemp[1] = (uint8_t)(usValue >> 24);
    ucTemp[2] = (uint8_t)(usValue >> 16);
...
bool VL53L0X::getSpadInfo(uint8_t *pCount, uint8_t *pTypeIsAperture)
{
int iTimeout;
