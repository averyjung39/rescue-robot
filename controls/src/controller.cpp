...
void Controller::actuate(const planning::ControlCommand &control_cmd) {
    std::pair<float, float> right_left_velocities = getVelocities(control_cmd);
...
std::pair<float, float> Controller::getVelocities(const planning::ControlCommand &control_cmd) const {
    const float &command_type = control_cmd.command_type;
    const bool &direction_is_right = control_cmd.direction_is_right;
    const float &speed_r = control_cmd.speed_r;
    const float &speed_l = control_cmd.speed_l;

    std::pair<float, float> right_left_velocities = std::make_pair(_rpm_right, _rpm_left);

    if (command_type == planning::ControlCommand::STRAIGHT_LINE) {
        // right is forward
    ...
    } else if (command_type == planning::ControlCommand::STOP) {
        right_left_velocities.first = rampVelocity(0, true);
        right_left_velocities.second = rampVelocity(0, false);
    } else if (command_type == planning::ControlCommand::TURN_ON_SPOT) {
        int direction_sign = direction_is_right ? -1 : 1;
        right_left_velocities.first = rampVelocity(direction_sign*speed_r, true);
        right_left_velocities.second = rampVelocity(direction_sign*speed_l, false);
    } else {
